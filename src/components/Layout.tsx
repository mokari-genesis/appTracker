...
import { Users, Tag, Package, Gavel, LogOut, User, ChartLine } from 'lucide-react'
      ...
      <div className='w-80 bg-white border-r border-gray-200 shadow-sm flex flex-col'>
        ...
        <div className='flex items-center h-20 px-6 border-b border-gray-200 bg-gradient-to-r from-slate-900 to-slate-800'>
          <div className='flex items-center'>
            ...
            <span className='ml-3 text-2xl font-bold text-white'>AuctionTracker</span>
          </div>
        </div>
        ...
        <nav className='flex-1 px-6 py-8 space-y-3'>
          {navigation.map(item => {
            ...
            return (
              ...
              >
                <item.icon className={cn('h-7 w-7 mr-4', isActive ? 'text-white' : 'text-gray-600')} />
                <span className={cn(isActive ? 'text-white' : 'text-gray-600')}>{item.name}</span>
              </button>
            )
          })}
        </nav>
